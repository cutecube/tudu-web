 ...
 * 通讯录控制器
 *
 * @version $Id: ContactController.php 2613 2013-01-06 09:26:12Z cutecube $s
 */
...
class ContactController extends TuduX_Controller_Base
{
	...
	public function viewAction()
	{
		...
		if ($email) {
			...
			$imClient = new Oray_Im_Client($config['host'], $config['port']);

			$imStatus = $imClient->getUserStatus($profile->address);

			if(isset($imStatus[$profile->address])) {
			    $imStatus = $imStatus[$profile->address];
			}
