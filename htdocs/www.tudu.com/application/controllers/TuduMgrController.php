 ...
 *
 * @version $Id: TuduMgrController.php 2771 2013-03-11 03:50:01Z cutecube $
 */
class TuduMgrController extends TuduX_Controller_Base
{
    ...
    public function rejectAction()
    {
        ...
        foreach ($tuduIds as $tuduId) {
            ...
            $tuduStatus = $this->manager->rejectTudu($tuduId, $this->_user->uniqueId, $isFlow);
            if (false !== $tuduStatus) {
                $success ++; //记录次数
                ...
                if (!empty($updateTudu)) {
                    $this->manager->updateTudu($tuduId, $updateTudu);
                }
                ...
                if ($tudu->flowId) {
                    ...
                    if (isset($steps[$tudu['stepid']])) {
                        ...
                        if (0 !== strpos($tudu['stepid'], 'F-')) {
                            //$daoStep->updateStep($tudu['tuduid'], $step['prevstepid'], array('nextstepid' => $step['nextstepid']));
                            $daoStep->deleteStep($tudu['tuduid'], $tudu['stepid']);
                            $isFrowardStep = true;
                        } else {
                            // 更新当前步骤状态
                            $daoStep->updateStep($tudu['tuduid'], $tudu['stepid'], array('status' => 3));
                            $daoStep->updateUser($tudu['tuduid'], $tudu['stepid'], $this->_user->uniqueId, array('status' => 3));
                        }
                        ...
                        if (isset($steps[$prevId])) {
                            ...
                            if ($isFrowardStep) {
                                $prevUpdate['nextstepid'] = $step['nextstepid'];
                            }

                            $users = $daoStep->getUsers($tudu['tuduid'], $prev['stepid']);
                            // 上一步骤系审批
                            if ($prev['type'] == Dao_Td_Tudu_Step::TYPE_EXAMINE) {
                                ...
                                foreach ($users as $item) {
                                    ...
                                    if ($item['status'] > 1) {
                                        $updateStatus = true;
                                        $prevUpate['status'] = 0;
                                        $daoStep->updateUser($tudu['tuduid'], $prev['stepid'], $item['uniqueid'], array('status' => 1));
                                    }
                                }
                            ...
                            // 上一步骤系执行
                                ...
                                foreach ($users as $item) {
                                    ...
                                    if ($item['status'] > 1) {
                                        $daoStep->updateUser($tudu['tuduid'], $prev['stepid'], $item['uniqueid'], array('status' => 1));
                                        $this->manager->updateProgress($tudu['tuduid'], $item['uniqueid'], 0);
                                        if ($tudu['parentid']) {
                                            $this->manager->calParentsProgress($tudu['parentid']);
                                        }
                                        $updateStatus = true;
                                ...
                                $this->manager->updateTudu($tudu['tuduid'], array('to' => Tudu_Tudu_Storage::formatReceiver($to)));
                            }
                            // 更新上一步骤状态
                            if ($updateStatus) {
                                $daoStep->updateStep($tudu['tuduid'], $prev['stepid'], $prevUpdate);
                            }
                    ...
                    // 更新图度stepId
                    $updateParams = array();
                    $updateParams['stepid'] = isset($stepId) ? $stepId : '^break';
                    if ($stepId == '^break' || $stepId == '^head') {
                        $tudu['to'] = Tudu_Tudu_Storage::formatRecipients($tudu['from'][3] . ' ' . $tudu['from'][0]);
                        $accepters = $this->manager->getTuduAccepters($tudu['tuduid']);
                        foreach ($tudu['to'] as $key => $val) {
                            foreach ($accepters as $accepter) {
                                $info = explode(' ', $accepter['accepterinfo']);
                                if ($key == $info[0]) {
                                    $tudu['to'][$key]['percent'] = $accepter['percent'];
                                }
                            }
                        }
                        $updateParams['to'] = $tudu['from'][3] . ' ' . $tudu['from'][0];
                    ...
                    if (!$tudu->reviewer) {
                        ...
                        foreach ($accepters as $item) {
                            ...
                            if (!empty($to) && !array_key_exists($email, $to)
                                    && $this->manager->getChildrenCount($tudu->tuduId, $item['uniqueid']) <= 0)
                            {
                    ...
                    $this->manager->markAllUnRead($tudu->tuduId);
                    ...
                    if ($stepId != '^break' && $stepId != '^head' && isset($steps[$stepId]) && $steps[$stepId]['type'] != Dao_Td_Tudu_Step::TYPE_EXAMINE) {
                        if ($steps[$stepId]['type'] == Dao_Td_Tudu_Step::TYPE_EXECUTE) {
                            $users = $daoStep->getUsers($tudu->tuduId, $stepId);
                            foreach ($users as $item) {
                                $this->manager->acceptTudu($tudu->tuduId, $item['uniqueid'], null);
                            }
                            $updateParams['acceptmode'] = 0;
                        } else if ($steps[$stepId]['type'] == Dao_Td_Tudu_Step::TYPE_CLAIM) {
                            $updateParams['acceptmode'] = 1;
                            $updateParams['accepttime'] = null;
                        }
                    }
...
HTML;
    ...
    public function claimAction()
    {
        ...
        // 获取步骤
        $step = $this->manager->getStep($tuduId, $tudu->stepId);
        // 判断当前是否为认领操作
        ...
        $ret = $this->manager->claimTudu($tuduId, $this->_user->orgId, $this->_user->uniqueId);
        ...
        $postId = $this->manager->createPost($params);
        $this->manager->sendPost($tuduId, $postId);

        // 标记为未读
        $this->manager->markAllUnRead($tuduId);
...
<a href="http://{$this->_request->getServer('HTTP_HOST')}/frame#m=view&tid=%s&page=1" target="_blank" _tid="{$tuduId}">%s</a><br />
