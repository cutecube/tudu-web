...
class ComposeController extends TuduX_Controller_Base
{
    ...
    public function replyAction()
    {
        ...
        $params = array(
            ...
            'content'    => strip_tags($post['content'], $this->_enableTags),
        ...
        if ($type != 'save') {
            ...
            // 逐级执行，100%后更新下一步骤执行人
            if ($tuduPercent >= 100 && empty($tudu->flowId)) {
                $tudu   = $tudu->toArray();
                $update = array();
                $stepId = $tudu['stepid'];
                ...
                if (isset($steps[$stepId])) {
                    // 更新当前步骤状态
                    $daoStep->updateStep($tudu['tuduid'], $stepId, array('status' => 2));
                    $daoStep->updateUser($tudu['tuduid'], $stepId, $this->_user->uniqueId, array('status' => 2, 'percent' => $tuduPercent));

                    // 指定到下一步骤
                    $nextId = $steps[$stepId]['nextstepid'];
                    if ($nextId == '^end') {
                        $stepId = '^end';
                    }

                    if (isset($steps[$nextId])) {
                        $nextStep      = $steps[$nextId];
                        ...
                        // 更新图度接收人信息
                        $update['to'] = Tudu_Tudu_Storage::formatReceiver($users);
                ...
                // 更新图度stepId
                $update['stepid'] = $stepId;
                $manager->updateTudu($tudu->tuduId, $update);
                // 更新进度
            ...
            // 工作流
            if ($tudu->flowId && $tuduPercent >= 100) {
                $tudu = $tudu->toArray();
                $tudu['cc'] = null;
                $stepId = $tudu['stepid'];
                $daoStep = $this->getDao('Dao_Td_Tudu_Step');
                ...
                if (isset($steps[$stepId])) {
                    // 更新当前步骤状态
                    $daoStep->updateStep($tudu['tuduid'], $stepId, array('status' => 2));
                    $daoStep->updateUser($tudu['tuduid'], $stepId, $this->_user->uniqueId, array('status' => 2));
                    // 指定到下一步骤
                    $nextId = $steps[$stepId]['nextstepid'];
                    if ($nextId == '^end') {
                        $stepId = '^end';
                ...
                if ($stepId == '^break') {
                    ...
                    // 更新图度接收人信息
                    $manager->updateTudu($tudu['tuduid'], array('to' => $tudu['from'][3] . ' ' . $tudu['from'][0]));
                }
                ...
                $manager->updateTudu($tudu->tuduId, $updateParams);
            }
