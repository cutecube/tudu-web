 ...
 *
 * @author Hiro
 * @version $Id: TuduController.php 2771 2013-03-11 03:50:01Z cutecube $
 */
...
class TuduController extends TuduX_Controller_Base
{
    ...
    public function indexAction()
    {
        ...
        $pageSize = isset($this->_user->option['pagesize']) ? $this->_user->option['pagesize'] : 20;
        $params = array();
    ...
    public function viewAction()
    {
        ...
        $pageSize  = isset($this->_user->option['replysize']) && intval($this->_user->option['replysize']) > 0 ? (int) $this->_user->option['replysize'] : 20;
        $query     = array();
        $votes     = null;
        ...
        if ($tudu->isDraft || $isForward || $isDivide || $isReview || $isInvite || $isApply) {
            ...
            if ($tudu->flowId) {
                ...
                $flows = $daoFlow->getFlows(array('orgid' => $this->_user->orgId, 'boardid' => $tudu->boardId), array('isvalid' => 1));
                ...
                foreach ($children as $key => $child) {
                    ...
                    if (empty($child['flowid']) && $child['stepid']) {
                        if ($child['stepid'] == '^end') {
                            ...
                            $stepUsers = $daoStep->getTuduStepUsers($child['tuduid']);

                            $isExceed = false;
                            $toIdx    = null;
                            foreach ($stepUsers as $u) {
                                if (!$isExceed && ($u['stepid'] == $child['stepid'] || $child['stepid'] == '^head')) {
                                    $isExceed = true;
                                }
        ...
        foreach ($posts as $key => $post) {
            ...
            if (count($postTime) <= 1) {
                ...
                for ($i=0; $i<count($postTime);$i++) {
                    if ($postTime[$i] == $post['createtime']) {//echo $postTime[$i];echo '<br />';
                        if (empty($postTime[$i+1])) {$postTime[$i+1] = time();}
                        foreach ($logs as $log) {
                            if ($log['logtime'] > $post['createtime'] && $log['logtime'] <= $postTime[$i+1]) {
                               $kk[] = $log;
                            }
                        }
                    }
    ...
    public function modifyAction()
    {
        ...
        if ($tuduId) {
            ...
            if ($tudu->flowId) {
                ...
                $flow = $daoFlow->getFlow(array('flowid' => $tudu->flowId))->toArray();
            ...
            if ($tudu->isTuduGroup && (!$tudu->to || in_array($this->_user->userName, $tudu->accepter))) {
                ...
                foreach ($children as $key => $child) {
                    ...
                    if ($child['status'] == 3) {
                        $chClaim = $daoStep->getStep(array(
                            'tuduid' => $child['tuduid'],
                            'type' => 2
                        ));

                        if ($chClaim) {
                            $claimAccepters = $daoStep->getUsers($child['tuduid'], $chClaim->stepId);
            ...
            if ($tudu->status == 3 && !$tudu->isTuduGroup) {
                $claim = $daoStep->getStep(array(
                    'tuduid' => $tuduId,
                    'type' => 2
                ));

                if ($claim) {
                    $claimAccepters = $daoStep->getUsers($tuduId, $claim->stepId);
            ...
            // 逐级执行，执行人填充  审批人填充
                ...
                foreach ($stepUsers as &$u) {
                    ...
                    if ($u['type'] == Dao_Td_Tudu_Step::TYPE_EXAMINE && (int) $u['stepstatus'] != 4) {
                        if ($u['status'] != 2) {
                            if ($processindex == $u['processindex'] && ($u['stepid'] == $tudu['stepid'] || $tudu['stepid'] == '^head')) {
                                $reviewer[]['userinfo'] = '+';
                            }
                    ...
                    if ($type == 'task' && $isExceed && $u['type'] == Dao_Td_Tudu_Step::TYPE_EXECUTE && (int) $u['stepstatus'] != 4) {
                        if (empty($toIdx)) {
                            $to[] = $u;
                            $toIdx = $u['ordernum'];
                            continue;
...
//            $vote['enabled'] = !$vote['isvoted'] && !$vote['expired'];
    ...
    public function formatStepsToHtml($steps)
    {
        ...
        $prevUser = array(array('email' => $this->_user->userName));
        ...
        foreach ($steps as $key => $step) {
            ...
            if ($step['type'] == 1) {
                    ...
                    if ($item == '^upper') {
                        ...
                        if (!empty($prevUser)) {
                            ...
                            foreach ($prevUser as $user) {
                                $m = $this->_getUpper($user['email']);
                                if (empty($m)) {
                                    $moderator = null;
                                    break;
                                }
                                $moderator = array_merge($moderator, $m);
                            }
                        ...
                        $html[] = '<span title="'.$userName.'">'.$trueName.'('.$this->lang['future_review'].')'.'</span>';
                    }

                    if ($index == $count && $stepIdx != $countSteps) {
                        $html[] = '<span class="icon icon_flow_arrow"></span>';
                    }
                }
                ...
                if ($count > 1) {
                    $html[] = '<span title="'.$this->formatName($arr).'">'.$this->lang['multi_accepter'].'</span>';
                    $pu = array();
                    foreach ($arr as $u) {
                        list ($userName, ) = explode(' ' , $u);
                        $pu[] = array('email' => $userName);
                    }
                    $prevUser = $pu;
                } else {
                    list ($userName, $trueName) = explode(' ', $arr[0]);
                    $html[] = '<span title="'.$userName.'">'.$trueName.'</span>';
                    $prevUser = array(array('email' => $userName));
                }
    ...
    private function _getModifyParams($isExternal = false)
    {
        ...
        $tuduFields = array(
            'type', 'bid', 'classid', 'subject', 'subject', 'to', 'cc', 'priority',
