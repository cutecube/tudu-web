 ...
 * @author     Oray-Yongfa
 * @version    $Id: FlowController.php 2484 2012-12-07 10:36:54Z cutecube $
 */
class FlowController extends TuduX_Controller_Base
{
    ...
    public function modifyAction()
    {
        ...
        if ($flowId) {
            ...
            foreach ($flow['steps'] as &$step) {
                ...
                if ($step['type'] == 1) {
                    $step['fusers'] = Tudu_Tudu_Storage::formatReviewer($step['users']);
                } else {
                    $step['fusers'] = Tudu_Tudu_Storage::formatRecipients($step['users']);
                }
            }
    ...
    public function saveAction()
    {
        ...
        if (!$flowId) {
            ...
            $params['steps'] = Dao_Td_Flow_Flow::formatXml($params['flowid'], $this->formatSteps($post));
        ...
        // 更新
            ...
            if ($isValid) {
                ...
                if ($isNew) {
                    ...
                    $params['flowid'] = Dao_Td_Flow_Flow::getFlowId();
                    $params['steps']  = Dao_Td_Flow_Flow::formatXml($params['flowid'], $this->formatSteps($post, $flowId));
                    // 创建新的工作流
                    $ret = $daoFlow->createFlow($params);
                    $flowId = $ret;
                    ...
                    $params['steps']  = Dao_Td_Flow_Flow::formatXml($flowId, $this->formatSteps($post));
    ...
    public function formatSteps($post, $flowId = null)
    {
        ...
        foreach ($members as $member) {
            $steps[$post['order-' . $member]] = array(
                    'id'          => !empty($post['id-' . $member]) ? $post['id-' . $member] : Dao_Td_Flow_Flow::getStepId(),
        ...
        foreach ($steps as $key => $step) {
            ...
            $steps[$key]['prev'] = (in_array($pervIds[$key], $members)) ? $steps[$pervIds[$key]]['id'] : $pervIds[$key];
            $steps[$key]['next'] = $next;
        }
        ...
        if ($flowId) {
            ...
            foreach ($newSteps as $key => $step) {
                if (isset($newSteps[$key + 1]['id'])) {
                    $next = $newSteps[$key + 1]['id'];
                } else {
                    $next = '^end';
                }

                $newSteps[$key]['prev'] = (in_array($pervIds[$key], $members)) ? $newSteps[$pervIds[$key]]['id'] : $pervIds[$key];
