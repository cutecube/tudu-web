...
{{foreach from=$columns item=column}}
    ...
    <td class="title_line"><div class="space">{{if !$coreseek}}<a href="javascript:void(0);" onClick="Tudu.List.sort('?{{$query}}', 1,{{$sort[1]}});return false;">{{$LANG.subject}}{{if $sort[0]==1}}{{if $sort[1]==0}}↑{{else}}↓{{/if}}{{/if}}</a>{{else}}{{$LANG.subject}}{{/if}}</div></td>
  ...
  <tr>
    ...
    <td class="g_in">
      <table class="g_in_table" cellspacing="0" cellpadding="0">
        <tr>
          <td class="sender"><div class="space"><a _email="{{$tudu.sender}}"{{if $tudu.sender != '^system'}} href="/contact/view?email={{$tudu.sender}}&back={{$currUrl}}"{{/if}}>{{if $tudu.sender==$user.username}}{{$LANG.me}}{{else}}{{$tudu.from.0|default:'-'}}{{/if}}</a></div></td>
          <td class="subject">
          <div class="space">
          {{*草稿仅显示标题*}}
          {{if $tudu.isdraft}}
          ...
          {{else}}
            ...
            {{/if}}
            {{*主题前面的状态显示*}}
            {{if $tudu.selftudustatus == 3 && $tudu.type == 'task' && $label.labelalias == 'todo'}}
            {{assign var="statusKey" value="tudu_status_"|cat:$tudu.selftudustatus"}}
            ...
            {{/if}}
            {{if in_array($user.username, $tudu.accepter, true) && ($tudu.selftudustatus < 2 && !$tudu.selfaccepttime)}}
          ...
          <td class="deadline">
            ...
            {{else}}
            <cite>{{if $tudu.accepter}}<a _email="{{$tudu.accepter[0]}}" href="/contact/view?email={{$tudu.accepter[0]}}&back={{$currUrl}}">{{if $tudu.accepter.0==$user.username}}{{$LANG.me}}{{else}}{{$tudu.to[$tudu.accepter.0].0|default:'-'}}{{/if}}</a>{{else}}-{{/if}}</cite>
            {{/if}}
          ...
          <td class="rate">
            <div class="space">
            {{if $tudu.appid == 'attend' || ($tudu.flowid && 0 !== strpos('^', $tudu.flowid))}}
        ...
        <tr>
          ...
          <td class="subject">
              <div class="space">
                ...
                {{/if}}
                <a href="/tudu/view?tid={{$tudu.tuduid}}&back={{$currUrl}}" title="{{$tudu.subject|escape:'html'}}">{{if $tudu.displaydate && $tudu.type == 'meeting'}}[{{$tudu.starttime|date_time_format:'%Y%m%d'}}]{{/if}}{{$tudu.subject|escape:'html'|default:$LANG.null_subject}}</a>
                {{* 列表分页 *}}
                {{tudu_list_pagenav recordcount=$tudu.replynum+1 pagesize=20 url='/tudu/view' query='tid='|cat:$tudu.tuduid|cat:'&back='|cat:$currUrl}}
                ...
                {{if $tudu.type == 'meeting' && in_array($user.username, $tudu.accepter, true) && ($tudu.selftudustatus < 2 && !$tudu.selfaccepttime)}}
                <span class="tips_label tips_receive" style="margin-left:5px">
                    <span class="tips_label_tl"><span class="tips_label_tr"><span class="tips_label_tc"></span></span></span>
                    <span class="tips_label_body" style="text-align:center">{{$LANG.need_accept}}</span>
                    <span class="tips_label_bl"><span class="tips_label_br"><span class="tips_label_bc"></span></span></span>
...
{{if ($key == 'discuss' && $unupdatediscuss) || ($key == 'other' && $unupdateother) || $unupdatetodo[$key]}}
<div id="unupdate-{{$key}}" _type="{{$key}}" style="height:30px;line-height:30px;text-align:center;background-color:#ffffea;border-bottom:1px solid #EEEEEE;"><a name="show_outdate" href="javascript:void(0);">{{if $key == 'discuss'}}{{$LANG.longtime_no_update_discuss}}{{elseif $unupdatetodo[$key]}}{{$LANG.longtime_no_confirm_tudu}}{{else}}{{$LANG.longtime_no_update_tudu}}{{/if}}↓</a></div>
{{/if}}
</div>
{{/if}}
{{foreachelse}}
...
{{if $tudu.type == 'task'}}
  <tr>
    ...
    <td class="g_i">
      ...
      <a href="/tudu/view?tid={{$tudu.tuduid}}&newwin=1&back={{$currUrl}}" target="_blank" class="g_i_c Rr{{if !$tudu.isdraft || $tudu.istudugroup}}g{{/if}}u{{if $tudu.special == 1}}_c{{/if}}" title="{{$LANG.new_win_tips}}"></a>
      {{else}}
      <a href="/tudu/view?tid={{$tudu.tuduid}}&newwin=1&back={{$currUrl}}" target="_blank" class="g_i_c Rr{{if $tudu.istudugroup}}g{{/if}}{{if !$tudu.isread}}u{{/if}}{{if $tudu.special == 1}}_c{{/if}}" title="{{$LANG.new_win_tips}}"></a>
      {{/if}}
      <div class="g_i_r{{if $tudu.attachnum > 0}} el{{/if}}"></div>
    ...
    <td class="g_in">
      <table class="g_in_table" cellspacing="0" cellpadding="0">
        <tr>
          ...
          {{/if}}
          {{if in_array('replynum_viewnum', $columns)}}
          <td class="rate">
            <div class="space">
            <em>{{$tudu.replynum}}/{{$tudu.viewnum}}</em>
            </div>
