 ...
 * @author     Oray-Yongfa
 * @version    $Id: Apply.php 2771 2013-03-11 03:50:01Z cutecube $
 */
...
class Apps_Attend_Apply extends Apps_Attend_Abstract
{
    ...
    public function composeAction()
    {
        ...
        if (empty($post['categoryid'])) {
            return $this->json(false, $this->lang['invalid_categoryid']);
        }
        $categoryId = $post['categoryid'];

        if ($categoryId == '^checkin') {
            if (!array_key_exists('checkintype', $post)) {
                return $this->json(false, '请选择补签类型');
            }

            if ($post['checkintype'] == 0 && empty($post['starttime'])) {
                return $this->json(false, $this->lang['invalid_starttime']);
            } elseif ($post['checkintype'] == 1 && empty($post['endtime'])) {
                return $this->json(false, $this->lang['invalid_endtime']);
            }
        } else {
        ...
        $daoCategory = Tudu_Dao_Manager::getDao('Dao_App_Attend_Category', Tudu_Dao_Manager::DB_APP);
        // 检查申请类型
        $category = $daoCategory->getCategory(array('categoryid' => $categoryId, 'orgid' => $this->_user->orgId));

        if (null === $category) {
            return $this->json(false, $this->lang['category_not_exists']);
        }
        ...
        $fromTuduId = !empty($post['ftid']) ? $post['ftid'] : null;
        ...
        $params = array(
            ...
            'cc'         => Tudu_Tudu_Storage::formatRecipients($post['cc']),
        ...
        if (null !== $fromTuduId) {
            $params['tuduid'] = $fromTuduId;
        }

        // 添加暗送
        if (!empty($post['target'])) {
            $info = explode(' ', $post['target']);
            if ($info[0] != $this->_user->userName) {
                $params['bcc'] = Tudu_Tudu_Storage::formatRecipients($post['target']);
            }
        }

        $tudu = new Tudu_Model_Tudu_Entity_Tudu($params);

        // 附件 / 图片
        if (!empty($post['attach']) && is_array($post['attach'])) {
            foreach ($post['attach'] as $fid) {
                $tudu->addAttachment($fid, true);
            }
        }
        if (!empty($post['file']) && is_array($post['file'])) {
            foreach ($post['file'] as $fid) {
                $tudu->addAttachment($fid, false);
            }
        }
        ...
        if (null === $fromTuduId) {
            ...
            if ($categoryId != '^checkin' ) {
                $applyParams['checkintype'] = null;
                $applyParams['isallday']  = $post['isallday'] ? 1 : 0;
                $applyParams['starttime'] = strtotime($post['starttime']);
                $applyParams['endtime']   = strtotime($post['endtime']);
                $applyParams['period']    = (float) $post['period'];
            } else {
                $applyParams['isallday']    = null;
                $applyParams['checkintype'] = $post['checkintype'] ? 1 : 0;
                $applyParams['period']      = null;
                if ($post['checkintype'] == 0) {
                    $applyParams['starttime'] = strtotime($post['starttime']);
                    $applyParams['endtime']   = null;
                } elseif ($post['checkintype'] == 1) {
                    $applyParams['endtime']   = strtotime($post['endtime']);
                    $applyParams['starttime'] = null;
                }
            }

            $apply = new Tudu_Model_App_Attend_Tudu_Apply($applyParams);
            if (!empty($post['target'])) {
                $target = trim($post['target']);
                list ($userName, $trueName) = explode(' ', $target);

                $addressBook = Tudu_AddressBook::getInstance();
                $user = $addressBook->searchUser($this->_user->orgId, $userName);

                $apply->setAttribute(array(
                    'uniqueid' => $user['uniqueid'],
                    'userinfo' => $trueName . ' ' . $userName
                ));
                $tudu->setAttribute('target', $userName . ' ' . $trueName);
            } else {
                $apply->setAttribute(array(
                    'uniqueid' => $this->_user->uniqueId,
                    'userinfo' => $this->_user->trueName . ' ' . $this->_user->userName
                ));
                $tudu->setAttribute('target', $this->_user->userName . ' ' . $this->_user->trueName);
            }

            $tudu->addExtension($apply);

            $applyStatus = 0;
        } else {
            $daoApply = Tudu_Dao_Manager::getDao('Dao_App_Attend_Apply', Tudu_Dao_Manager::DB_APP);

            $recordApply = $daoApply->getApply(array('tuduid' => $fromTuduId));

            if (null === $recordApply) {
                return $this->json(false, $this->lang['apply_not_exists']);
            }

            $applyStatus = $recordApply->status;

            $applyParams = array('applyid' => $recordApply->applyId);
            if ($isSend) {
                $applyParams['status'] = 0;
            }
            if ($recordApply->status == -1) {
                $categoryId = $post['categoryid'];
            } else {
                $categoryId = $recordApply->categoryId;
            }
            $applyParams['categoryid'] = $categoryId;
            if ($categoryId != '^checkin' ) {
                $applyParams['checkintype'] = null;
                $applyParams['isallday']  = $post['isallday'] ? 1 : 0;
                $applyParams['starttime'] = strtotime($post['starttime']);
            ...
            $apply = new Tudu_Model_App_Attend_Tudu_Apply($applyParams);
            $apply->setAttribute(array(
                'uniqueid' => $recordApply->uniqueId,
                'userinfo' => $recordApply->userInfo
            ));
            $userInfo = explode(' ', $recordApply->userInfo);
            $tudu->setAttribute('target', $userInfo[1] . ' ' . $userInfo[0]);
        ...
        $period = 0;
        if ($categoryId != '^checkin') {
            $startTime = $apply->getAttribute('starttime');
            $endTime   = $apply->getAttribute('endtime');
            $period    = Oray_Function::dateDiff('day', $startTime, $endTime) + 1;
        }

        if ($action == 'send' && ($applyStatus == 0 || $applyStatus == 3)) {
            $tuduFlow = new Tudu_Model_Tudu_Entity_Extension_Flow();

            $flow     = $category->flowSteps;
            $steps    = $flow['steps'];
            $stepArr  = array();
            $orderNum = 1;
            $prevId   = null;
            ...
            foreach ($steps as $item) {
                ...
                $step['step'] = array(
                    ...
                    'prevstepid' => '^head',
                    'nextstepid' => $item['next'],
                    'ordernum'   => $orderNum ++
                );

                if (!empty($item['branches'])) {
                    foreach ($item['branches'] as $branch) {
                        if (($branch['start'] <= 0 || $branch['start'] <= $period) && ($branch['end'] <= 0 || $branch['end'] >= $period)) {
                            $users = $branch['users'];
                            break;
                        }
                    }
            ...
            if (empty($reviewSteps)) {
                return $this->json(false, $this->lang['apply_category_error_apply_time']);
            }

            $count = count($reviewSteps);
            $reviewSteps[$count-1]['step']['nextstepid'] = '^end';

            foreach ($reviewSteps as $item) {
                $step  = new Tudu_Model_Tudu_Entity_Step($item['step']);
                $step->setAttribute('users', $item['users']);
                $tuduFlow->addStep($step);
            }

            $tudu->addExtension($tuduFlow);
        }


        try {
            Tudu_Model_Tudu_Compose::registerExtensionNameSpace('Tudu_Model_App_Attend_Tudu_Extension');

            $composeModel = new Tudu_Model_Tudu_Compose();

            // 调用发送模型方法进行发送
            $composeModel->registerResource(Tudu_Model_Tudu_Compose::RESOURCE_NAME_USER, $this->_user);
            ...
            if ($isSend) {
                ...
                $composeModel->registerResource(Tudu_Model_Tudu_Compose::RESOURCE_NAME_HTTPSQS, $httpsqs);
            }

            $tuduId = $composeModel->{$action}($tudu, $output);

        } catch (Tudu_Model_Tudu_Exception $e) {
            $message = str_replace(' ', '_', strtolower($e->getMessage()));
            return $this->json(false, $this->lang[$message]);
        }

        $message = $action == 'save' ? 'apply_save_success' : 'apply_send_success';

        $output['tuduid'] = $tudu->tuduId;
        $this->json(true, $this->lang[$message], $output);
    }
    ...
    public function viewAction()
    {
        ...
        // 回复内容
        $page        = (int) $this->_request->getQuery('page');
        $pageSize    = $this->_user->option['replysize'];
    ...
    public function reviewAction()
    {
        ...
        if (null === $apply) {
            return $this->_this->json(false, $this->lang['apply_not_exists']);
        }

        $tudu = new Tudu_Model_Tudu_Entity_Tudu($params);

        $tudu->addExtension(new Tudu_Model_App_Attend_Tudu_Apply($apply->toArray()));

        try {
            Tudu_Model_Tudu_Compose::registerExtensionNameSpace('Tudu_Model_App_Attend_Tudu_Extension');
            Tudu_Model_Tudu_Compose::registerResource(Tudu_Model_Tudu_Abstract::RESOURCE_NAME_USER, $this->_user);

            $composeModel = new Tudu_Model_Tudu_Compose();

            $composeModel->review($tudu);
            ...
            $daoLog->createLog(array(
                ...
                'targettype' => Dao_Td_Log_Log::TYPE_TUDU,
                'targetid'   => $tuduId,
                'action'     => Dao_Td_Log_Log::ACTION_TUDU_REVIEW,
                'detail'     => serialize(array('agree' => $isAgree ? 1 : 0)),
                'privacy'    => 0
            ));

        } catch (Tudu_Model_Tudu_Exception $e) {
            return $this->_this->json(false, $e->getMessage());
        }
