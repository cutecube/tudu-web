...
Tudu.removeLabel = function(tuduId, label, labelid) {
	$.ajax({
        ...
        success: function(ret) {
            ...
            var obj = $('#children-list #tudu-' + tuduId);
            if (obj.size()) {
		        ...
		        for (var i = 0, c = lbs.length; i < c; i++) {
		        	if (lbs[i] == labelid) {
		        		continue ;
		        	} else if (undefined !== Tudu.View._labels[lbs[i]]) {
		        		nlbs.push(lbs[i]);
		        		if (lc < 3) {
		        			Tudu.View.appendLabel(obj, lbs[i]);
		        		}
		        		lc ++;
		        	}
		        }
...
// 提交投票
Tudu.vote = function(form) {
	var voteId = form.attr('id').replace('voteform-', ''),
	    count = form.find('input[name="option-' + voteId + '[]"]:checked').size(),
		max   = form.find('input[name="maxchoices-' + voteId + '"]').val();

	if (count <= 0) {
	    return TOP.showMessage(TOP.TEXT.NO_SELECTED_OPTION);
	}

    if (parseInt(max) != 0 && count > max) {
        return TOP.showMessage(TOP.formatString(TOP.TEXT.TOO_MUCH_SELECTED, max));
    }
	...
	$.ajax({
		...
		success: function(ret) {
		   ...
		   if (ret.success) {
			   $('input[name="option-' + voteId + '[]"]').parent('td').css('padding', 0);
			   $('input[name="option-' + voteId + '[]"]').remove();

			   if (ret.data) {
				    var opts  = ret.data.options;
				    var total = ret.data.votecount;

				    for (var i = 0, c = opts.length; i < c; i++) {
				        var percent = total > 0 ? Math.round((opts[i].votecount / total) * 10000) / 100 : 0;
				        if (percent > 0) {
				        	$('#option-' + opts[i].optionid + '-percent-bar').show();
				        }
				        $('#option-' + opts[i].optionid + '-percent-bar').css('width', percent + '%');
				        $('#option-' + opts[i].optionid + '-percent').text('(' + percent + '%)');
				        $('#option-' + opts[i].optionid + '-count').text(opts[i].votecount);
				    }
				    form.find('button[type="submit"]').remove();
				    form.find('.vote-info').text(ret.message);
				    form.find('.vote-count').text(ret.data.countvoter);
				    form.find('.vote-visible').remove();
				    if (ret.data.privacy) {
				    	form.find('.vote-privacy').show();
				    }
				    form.find('div.vote-viewer').show();
					$('#download-' + voteId).show();
			   }
		   }
		},
		error: function(res) {
 ...
 * 删除联系人
 */
Tudu.deleteContact = function(contactId) {
 ...
 * 公告审批
 */
Tudu.noticeReview = function(tuduId, isAgree) {
	if (isAgree) {
		if (!confirm('您确认同意本次审批吗？')) {
			return false;
		}
	} else {
		if (!confirm('您确认不同意本次审批吗？')) {
			return false;
		}
	}
	...
	$.ajax({
        type: 'POST',
        dataType: 'json',
        data: {tid: tuduId, ftid: tuduId, action: 'send', review: 1, agree: isAgree, type: 'notice'},
        url: '/compose/send',
        success: function(ret) {
        	if (ret.success) {
        		location.reload();
 ...
 * @param {Object} flowId
 */
Tudu.FlowChart = function(flowId) {
	...
	var chartWin = TOP.appendWindow('chart-' + flowId, chartWinTpl, {
        width: 380,
        draggable: true,
        onShow: function() {
            // 取消、关闭窗口
            chartWin.find('a.icon_close, button[name="close"]').bind('click', function(){
                chartWin.close();
                return false;
            });

            chartWin.find('.pop_body iframe').attr('src', '/flow/chart?flowid=' + flowId);
        },
        onClose: function() {
            chartWin.destroy();
        }
...
Tudu.View = {
	...
	init: function(tuduId, back, currUrl) {
		...
		// 内容链接处理
		$('.todo_info a:not([href="javascript:void(0)"])').attr('target', '_blank');
		// 返回
		...
		function onResize() {
			...
			$('.post-content img').each(function(){
				...
				if (picWidth > bodyWidth) {
					$(this).css({'width': document.body.offsetWidth - 100});
				}
			});
        }
		...
		$('button[name="merge"]').click(function(){
			if (mergeDialog === null) {
				mergeDialog = new Tudu.mergeGroup({
					id: 'merge-dialog',
					tid: tuduId,
					jq: TOP.getJQ()
				});
		...
		$('button[name="modify"]').click(function(){
			var url = '/tudu/modify?tid=' + tuduId + '&back=' + currUrl;
			if (Tudu.View._isNewWin) {
				url += '&newwin=1';
			}
			location = url;
		...
		$('button[name="agree"]').bind('click', function(){
			...
			if (type == 'notice') {
				Tudu.noticeReview(tuduId, 1);
			} else {
				//Tudu.review(tuduId, '1');
				Tudu.ReviewWin.show({
					title: TOP.TEXT.REVIEW_CONFIRM,
					tips: TOP.TEXT.CONFIRM_TO_REVIEW_AGREE,
					next: (Tudu.View.flowId === null && !Tudu.View.sameReview) ? true : false,
					agree: 1,
					tids: [tuduId]
				});
			}
		...
		$('button[name="disagree"]').bind('click', function(){
			var type = $(this).attr('_type');
			if (type == 'notice') {
				Tudu.noticeReview(tuduId, 0);
			} else {
				//Tudu.review(tuduId, '0');
				Tudu.ReviewWin.show({
					title: TOP.TEXT.REVIEW_CONFIRM,
	    ...
	    $('button[name="cancel"]').click(function(){
	        if (!confirm(TOP.TEXT.CONFIRM_CANCEL_TUDU)) {
	            return ;
	        }
	    ...
	    // 投票
	    var voteform = $('form[name="voteform"]');
		if (voteform.size()) {
			...
			voteform.each(function(){
				$(this).find('.view-voter').bind('click', function(){
					var voteId = $(this).attr('_voteid');
					if (voterDialog === null) {
						voterDialog = new Tudu.VoterViewer({id: 'voter-dialog', tid: tuduId, vid: voteId, back: currUrl});
			        }
					
					voterDialog.setParam('vid', voteId);
					voterDialog.show();
				});
			});
		}
	 ...
	 * 初始化楼层信息
	 */
	initFloor: function(obj) {
		var _o = this;
		if (typeof obj == 'undefined') {
			obj = '.post-content';
		}
		$(obj).find('a[_jumpfloor^="FLOOR:"]').each(function(){
			var floor = $(this).attr('_jumpfloor').replace('FLOOR:', '');
			floor = floor.split('|');
			$(this).bind('click', function(){
				_o.jumpFloor(floor[0], floor[1]);
			});
			
			var isShow = $(this).attr('_initfloor'),
			    currentFloor = $(this).parents('div.todo_expand').attr('_floor');
			if (isShow) {
				if (obj != '.post-content') {
					$(this).parent().find('.show_floor, .floor_div').remove();
				}
				$(this).parent('p').attr('_currfl', currentFloor);
				$('<a class="floor_f show_floor" style="margin-left:5px;" href="javascript:void(0)">[' + TOP.TEXT.SHOW_FLOOR + ']</a>').bind('click', function(){
					Tudu.View.toggleFloor($(this).closest('p[_currfl="' + currentFloor + '"]'), floor[0], floor[1]);
				}).appendTo($(this).parent());
				$('<div class="floor_div" _fl="' + floor[0] + '" _empty="1" style="display:none;margin-top:10px;"></div>').appendTo($(this).parent());
			}
		});
	},
	 ...
	 * @param {Object} pid
	 */
	jumpFloor: function(floor, pid) {
		if ($('#post-' + pid).size()) {
			window.scrollTo(0,$('#post-' + pid).offset().top - 35);
		} else {
			if ($('div[_floor="' + floor + '"]').size()) {
				window.scrollTo(0,$('div[_floor="' + floor + '"]').offset().top - 35);
				return ;
			}
			
			location = '/tudu/view?tid=' + Tudu.View._tuduId + '&floor=' + floor;
		}
	},
	 ...
	 * @param {Object} pid
	 */
	toggleFloor: function(obj, floor, pid) {
		if (obj.find('.floor_div').attr('_empty') == 1) {
			if ($('#post-' + pid).size()) {
				$('#post-' + pid).find('.post-content').clone().removeClass().appendTo(obj.find('.floor_div'));
				obj.find('.floor_div:first').addClass('show').show();
				obj.find('.show_floor:first').text('[' + TOP.TEXT.HIDE_FLOOR + ']');
				obj.find('.floor_div:first').attr('_empty', 0);
				Tudu.View.initFloor(obj.find('.floor_div:first'));
			} else {
				Tudu.View.getFloors(obj, floor, pid);
			}
		} else {
			if (obj.find('.floor_div:first').hasClass('show')) {
				obj.find('.floor_div:first').removeClass('show').hide();
				obj.find('.show_floor:first').text('[' + TOP.TEXT.SHOW_FLOOR + ']');
			} else {
				obj.find('.floor_div:first').addClass('show').show();
				obj.find('.show_floor:first').text('[' + TOP.TEXT.HIDE_FLOOR + ']');
			}
	 ...
	 * @param {Object} floor
	 */
	getFloors: function(obj, floor, pid) {
		$.ajax({
            type: 'GET',
            dataType: 'json',
            url: '/tudu/get-floors?tid=' + Tudu.View._tuduId + '&pid=' + pid,
            success: function(ret) {
                if (ret.success && ret.data) {
					if (ret.data.content === null) {
						$('div[_fl="' + floor + '"]').html(TOP.TEXT.FLOOR_IS_NULL).addClass('red');
					} else {
						$('div[_fl="' + floor + '"]').html(ret.data.content);
					}
					$('div[_fl="' + floor + '"]').attr('_empty', 0);
					Tudu.View.initFloor('div[_fl="' + floor + '"]');
					
					obj.find('div[_fl="' + floor + '"]').addClass('show').show();
					obj.find('.show_floor:first').text('[' + TOP.TEXT.HIDE_FLOOR + ']');
                }
            },
            error: function(res) {}
	...
	replyPost: function(postid) {
		...
		if (postid) {
		    html = '<div class="cite_wrap"><p><strong>{0}</strong><a class="floor_f" style="margin:0 5px" href="javascript:void(0)" _jumpfloor="FLOOR:{4}|{3}" _initfloor="1">{1}</a><span class="floor_f" style="margin-left:5px">{2}</span></p></div><p>&nbsp;</p>';
		    var post = $('#post-' + postid),
		        poster = post.find('strong.poster').attr('_poster'),
		        floor  = post.find('.floor').text(),
				floorNum = floor ? parseInt(floor) : 0;
		    floor = floor ? floor : TOP.TEXT.FIRST_POST;
		    
		    html = TOP.formatString(html, TOP.TEXT.REPLY, floor, poster, postid, floorNum);
		}
	...
	reference: function(postid) {
	    var html = '<div class="cite_wrap"><strong>{0}</strong><span class="floor_f">{1}</span><a class="floor_f" style="margin-left:5px;" href="javascript:void(0)" _jumpfloor="FLOOR:{5}|{4}">{2}</a><div>{3}</div></div><p>&nbsp;</p>';
	    var post = $('#post-' + postid);
		...
		$('#edit-floor').empty();
	    $('#edit-floor').html(content);
		$('#edit-floor').find('.show_floor').remove();
		$('#edit-floor').find('.floor_div').remove();
		
		content = $('#edit-floor').html();
	    floor = floor ? floor : TOP.TEXT.FIRST_POST;
	    html = TOP.formatString(html, TOP.TEXT.REFERENCE, poster, floor, content, postid, parseInt(floor));
	...
	appendLabelMore: function(obj) {
		...
		if (ct.size() > 0) {
			ct.append('<a href="javascript:void(0)" onclick="Tudu.View.expandLabels(\''+obj.attr('id')+'\', this)" class="list_label_indent list_label_more"></a>');
		}
	},
	...
	expandLabels: function(id, obj) {
		...
		if (b.hasClass('list_label_more')) {
			...
			for (var i = 0, c = l.length; i < c; i++) {
			    ...
			    var lb = Tudu.View.appendLabel(o, l[i]);
			}
...
Tudu.Reply = {
	...
	init: function(params) {
		...
		this._vform = new VirtualForm({
			...
			filter: {compare: '#postcontent, #content, #tid, #board, #type, #action, #savetime, #fpid'},
		...
		$('a[name="tpllist"]').click(function(e) {
			...
			Tudu.Template.showMenu(e, me._editor, boardId);
			TOP.stopEventBuddle(e);
		});
		...
		setTimeout(function(){me._vform.save();}, 1000);
	},
		...
		var d = this.menuDialog('pic-dia', {
	        ...
	        oncreate: function() {
	    		...
	    		$('#pic-modal button[name="confirm"]').click(function(){
	    			var url = $('#picurl').val();
	    			if (url) {
	    				me._editor.pasteHTML('<img src="'+url+'" alt="" /><br />', true);
	    			}
	    			d.hide();
	    		});
	...
	send: function(action, callback) {
		...
		if (isSend) {
			...
			if (!this.whileUploading(TOP.TEXT.WAITING_UPLOAD, function(){Tudu.Reply.send('send');})) {
				return ;
			}
			...
			if (this._attachment.isUploading()) {
				...
				this._attachment.setParam('upload_complete_handler', function(){
					...
					if (stats.files_queued == 0 && !stats.in_progress) {
						me._form.find(':input').removeAttr('disabled');
						if (typeof(callback) == 'function') {
							callback();
						}
					}
 ...
 * 添加到图度组
 */
Tudu.mergeGroup = function(params) {
    this._settings = $.extend({}, params);
};
Tudu.mergeGroup.tpl = '<div class="pop pop_linkman"><div class="pop_header"><strong>' + TOP.TEXT.APPEND_TO_GROUP + '</strong><a class="icon icon_close close"></a></div><div class="pop_body" style="padding:10px"><div class="merge_group"><div class="search_div"><input id="tudu_search" class="input_text" type="text" /><a class="icon icon_search_2"></a></div><div id="search-tudulist" class="search-list" style="display:none;"></div><div id="group-tree-ct"></div></div></div><div class="pop_footer"><button type="button" name="confirm" class="btn">' + TOP.TEXT.CONFIRM + '</button><button type="button" class="btn close">'+TOP.TEXT.CANCEL+'</button></div></div>';
Tudu.mergeGroup.prototype = {
	...
	init: function(){
		...
		_o.win = TOP.appendWindow(_o._settings.id, Tudu.mergeGroup.tpl, {
			width: 300,
            draggable: true,
			onShow: function() {
				_o.getTudus();

				_o.win.find('a.icon_close, button.close').bind('click', function(){
                    _o.win.close();
                    return false;
                });

				_o.win.find('button[name="confirm"]').bind('click', function(){
					_o.submitMerge();
				});
			},
			onClose: function() {
	...
	submitMerge: function() {
		var _o = this,
		    tuduId = _o.getSelected();
		if (!tuduId) {
			TOP.showMessage(TOP.TEXT.APPEND_GROUP_NULL_TIPS);
			return false;
		}
		...
		$.ajax({
            type: 'POST',
            dataType: 'json',
            data: {tid: _o._settings.tid, targetid: tuduId},
            url: '/tudu-mgr/merge.group',
            success: function(ret) {
				TOP.showMessage(ret.message, 5000, ret.success ? 'success' : null);
	...
	getTudus: function() {
        ...
        $.ajax({
            type: 'GET',
            dataType: 'json',
            url: '/tudu/tudu-groups',
            success: function(ret) {
                if (ret.success && ret.data) {
					_o.tuduList = ret.data.tudus;
                    _o.mergeTree = new _o._settings.jq.tree({
				        id: 'group-tree',
				        idKey: 'id',
				        idPrefix: 'group-',
				        cls: 'cast-tree',
				    ...
				    _o.mergeTree.appendTo(TOP.getJQ()('#group-tree-ct'));
					_o.initBoardPanel(ret.data.boards);
					_o.initTudus(ret.data.tudus);
					_o.initSearch();
                }
            },
            error: function(res) {}
	...
	initSearch: function() {
		var _o = this;
		TOP.getJQ()('#tudu_search').bind('click', function(e){
            TOP.stopEventBuddle(e);
        }).bind('keyup', function(){
			...
			if (keyword) {
				_o.isSearch = true;
				_o.search(keyword);
				TOP.getJQ()('#search-tudulist div').each(function(){
					$(this).bind('click', function(){
						var tuduId = $(this).find('a').attr('_tuduid');
						_o.select(tuduId);
					});
				});
			} else {
				_o.isSearch = false;
				TOP.getJQ()('#search-tudulist').empty();
				TOP.getJQ()('#search-tudulist').hide();
				TOP.getJQ()('#group-tree-ct').show();
			}
        });
	},
	...
	search: function(keyword) {
		var _o = this,
		    length = _o.tuduList.length,
			tudus = _o.tuduList,
			keyword = keyword.toLowerCase();
		
		_o.isSearch = true;
		TOP.getJQ()('#search-tudulist').empty();
		TOP.getJQ()('.tree .tree-node-leaf').removeClass('tree-node-selected');
		
		if (length) {
			for (var i = 0, c = length; i < c; i++) {
				if (tudus[i]['subject'].toLowerCase().indexOf(keyword) >= 0) {
					TOP.getJQ()('#search-tudulist')
					.append('<div><span class="icon_mg"></span><a _tuduid="'+tudus[i]['tuduid']+'" title="'+tudus[i]['subject']+'" href="javascript:void(0);">'+tudus[i]['subject']+'</a></div>');
				}
			}
		}
		
		TOP.getJQ()('#group-tree-ct').hide();
		TOP.getJQ()('#search-tudulist').show();
	},
	 ...
	 * @param {Object} boards
	 */
	initBoardPanel: function(boards) {
		var _o = this;
		if (_o.mergeTree !== null) {
			for (var i = 0, c = boards.length; i < c; i++) {
				if (boards[i].type != 'system') {
					var bid = boards[i].boardid.replace('^', '_');
					var node = new _o._settings.jq.treenode({
						data: {
							id: 'b-' + bid,
							name: boards[i].boardname
						},
						events: {
							click: function(e){
								_o.mergeTree.find(this.id.replace('group-', ''), true).toggle();
								TOP.stopEventBuddle(e);
							}
						}
					});
					
					if (boards[i].parentid) {
						if (boards[i].parentid.indexOf('^') != -1) {
							boards[i].parentid = boards[i].parentid.replace('^', '_');
						}
						var parent = _o.mergeTree.find('b-' + boards[i].parentid, true);
	 ...
	 * @param {Object} tudus
	 */
	initTudus: function(tudus) {
		var _o = this;
        if (_o.mergeTree !== null) {
			for (var j = 0, tl = tudus.length; j < tl; j++) {
				var tuduid = tudus[j].tuduid;
				var node = new _o._settings.jq.treenode({
					data: {
						id: 't-' + tuduid,
						name: '<span class="icon_mg"></span><a title="' + tudus[j].subject + '" _tuduid="' + tudus[j].tuduid + '" href="javascript:void(0);">' + tudus[j].subject + '</a>'
					},
					isLeaf: true,
					events: {
						mouseover: function(e){
							$(this).addClass('tree-node-over');
							TOP.stopEventBuddle(e)
						},
						mouseout: function(e){
							$(this).removeClass('tree-node-over');
							TOP.stopEventBuddle(e)
						},
						click: function(e){
							_o.select(this.id.replace('group-', ''));
							TOP.stopEventBuddle(e);
						}
					}
				});
				
				var boardid = tudus[j].boardid ? tudus[j].boardid : '_root';
				if (boardid.indexOf('^') != -1) {
					boardid = boardid.replace('^', '_');
				}
				
				var parent = _o.mergeTree.find('b-' + boardid, true);
				if (parent) {
					parent.appendChild(node);
				}
			}
	...
	select: function(id) {
		var _o = this,
		    panel = TOP.getJQ()('.tree');
		panel.find('.tree-node-leaf').removeClass('tree-node-selected');
		TOP.getJQ()('#search-tudulist div').removeClass('selected');
		
		if (_o.isSearch) {
			TOP.getJQ()('#search-tudulist div').find('a[_tuduid="'+id+'"]').parent().addClass('selected');
		} else {
			if (_o.mergeTree !== null) {
				var node = _o.mergeTree.find(id, true);
				if (node == null) {
					return;
				}
				
				TOP.getJQ()('#group-' + id).addClass('tree-node-selected');
			}
		}
	},
	...
	getSelected: function() {
		var _o = this;
		if (_o.isSearch) {
			var panel = TOP.getJQ()('#search-tudulist');
			if (panel.find('div.selected').size() <= 0) {
				return null;
			}
			var tuduId = panel.find('div.selected a').attr('_tuduid');
			return tuduId;
		} else {
			if (_o.mergeTree !== null) {
				var panel = TOP.getJQ()('.tree');
				if (panel.find('.tree-node-selected').size() <= 0) {
					return null;
				}
				var tuduId = panel.find('.tree-node-selected').attr('id').replace('group-t-', '');
				return tuduId;
			} else {
				return null;
			}
...
Tudu.VoterViewer.prototype = {
	...
	getOptions: function() {
		...
		$.ajax({
			type: 'GET',
			dataType: 'json',
			url: '/tudu/vote-options?tid=' + _o._settings.tid + '&voteid=' + _o._settings.vid,
			success: function(ret) {
				if (ret.success && ret.data) {
					var options = ret.data;
					    html = [];
					_o.win.find('div.pop_body').append('<table style="width:100%;"><tr height=25><td width="60" style="text-align:right;vertical-align:top;color:#000000;"><div style="margin-top:5px;"><strong>'+TOP.TEXT.VOTE_OPTIONS+TOP.TEXT.CLN+'</strong></div></td><td><div id="vote-options"></div></td></tr><tr height=25><td style="text-align:right;vertical-align:top;color:#000000;"><div style="margin-top:5px;"><strong>'+TOP.TEXT.VOTE_VOTER+TOP.TEXT.CLN+'</strong></div></td><td><div id="voters"></div></td></tr></table>');
					
					for(var i=0; i<options.length; i++) {
						html.push('<div class="vote_option" id="op-'+options[i].optionid+'">'+options[i].text+'</div>');
						_o.voters[options[i].optionid] = options[i].voters;
					}
					
					TOP.getJQ()('#vote-options').append(html.join(''));
					
					var firstOptionId = TOP.getJQ()("#vote-options .vote_option:first").attr('id').replace('op-', '');
					_o.appendVoters(firstOptionId);
					
					TOP.getJQ()('#vote-options .vote_option').each(function(){
						$(this).bind('click', function(){
							var optionId = $(this).attr('id').replace('op-', '');
  	...
  	appendVoters: function(optionId) {
		...
		TOP.getJQ()('#vote-options .vote_option').removeClass('option_selected');
		TOP.getJQ()('#op-' + optionId).addClass('option_selected');
...
Tudu.ReviewWin = {
	...
	_tpl: [
		'<form id="review-form" action="/compose/send" method="post" enctype="multipart/form-data">',
		'<div class="pop">',
	...
	show: function(params) {
		...
		Win.find('#review-form').submit(function(){
			...
			var content = editor ? editor.getSource() : '';
	...
	serialize: function(filter) {
		...
		if (typeof this._editors == 'object') {
			for (var k in this._editors) {
				...
				var val = this._editors[k].getSource();
				if (val) {
					val = val.replace(/<[^>]+>/, '').replace('&nbsp', '').replace(/\s+/, '');
				}
