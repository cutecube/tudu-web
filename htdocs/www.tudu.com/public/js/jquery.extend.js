...
/* tree */
(function(a){a.tree=function(c){c=c||{};for(var b in c){this[b]=c[b]}this.init()};a.tree.prototype={ele:null,id:null,isAsync:false,childrenKey:"children",idKey:"id",idPrefix:"tree-",baseCls:"tree",nodes:null,init:function(){this.ele=a("<div>");this.ele.addClass(this.baseCls);this.nodeList=a("<ul>");this.nodeList.addClass(this.baseCls+"-root");this.ele.append(this.nodeList);if(this.id){this.ele.attr("id",this.id)}if(this.cls){this.ele.addClass(this.cls)}this.nodes=[]},getNode:function(h,b,f){if(!f){f=this.idKey}var d=null;for(var e=0,g=this.nodes.length;e<g;e++){if(this.nodes[e].get(f)==h){return this.nodes[e]}if(b){d=this.nodes[e].find(h,true,f);if(d!=null){return d}}}return null},find:function(d,b,c){return this.getNode(d,b,c)},search:function(b){var j=[];for(var f=0,m=this.nodes.length;f<m;f++){var g=0,n=0;for(var e in b){n++;if(b[e]==this.nodes[f].get(e)){g++}}if(g>0&&g==n){j.push(this.nodes[f])}var h=this.nodes[f].search(b);for(var e=0,d=h.length;e<d;e++){j.push(h[e])}}return j},appendTo:function(b){this.ele.appendTo(b);return this},appendNode:function(b){this.nodes.push(b);b.setTree(this);this.nodeList.append(b.ele);return this},removeNode:function(c){var b=this.find(c,true);if(b){b.remove()}return this},clear:function(){for(var b in this.nodes){this.nodes[b].remove();delete this.nodes[b]}this.nodes=[]}};a.treenode=function(c){c=c||{};for(var b in c){this[b]=c[b]}this._children=[];this.event={};this.init()};a.treenode.prototype={ele:null,nodeBody:null,id:null,_childList:null,tree:null,data:{},parent:null,depth:0,isLeave:false,event:null,autoEcIcon:true,init:function(){var b=this;if(!this.tree){return}this.ele=a("<li>");this.ele.addClass(this.tree.baseCls+"-node");this.nodeBody=a("<div>");this.nodeBody.addClass(this.tree.baseCls+"-node-el");this.indent=a("<span>");this.indent.addClass(this.tree.baseCls+"-node-indent");for(var d=0;d<this.depth;d++){this.indent.append("<span></span>")}this.nodeBody.append(this.indent);if(!this.isLeaf){this.nodeBody.append('<span class="'+this.tree.baseCls+"-ec-icon "+this.tree.baseCls+'-elbow-plus"></span>');this.nodeBody.find("."+this.tree.baseCls+"-ec-icon").click(function(f){b.toggle();f.cancelBubble=true;if(f.stopPropagation){f.stopPropagation()}});if(this.autoEcIcon){this.nodeBody.find("."+this.tree.baseCls+"-ec-icon").css({visibility:"hidden"})}}else{this.ele.addClass(this.tree.baseCls+"-node-leaf");this.nodeBody.append('<span class="'+this.tree.baseCls+'-ec-icon"></span>')}if(this.tree.template){this.content=this.tree.template}content=this._formatEl(this.content);this._childList=a("<ul>");this._childList.hide();if(this.events){for(var c in this.events){this.ele.bind(c,this.events[c])}}this.nodeBody.append(content);this.ele.append(this.nodeBody);this.ele.append(this._childList);if(this.data[this.tree.idKey]!=undefined){this.ele.attr("id",this.tree.idPrefix+this.data[this.tree.idKey])}},appendChild:function(b){this._children.push(b);b.parent=this;b.depth=this.depth+1;b.setTree(this.tree);this._childList.append(b.ele);this.nodeBody.find("."+this.tree.baseCls+"-ec-icon").css({visibility:"visible"})},insertBefore:function(d,c){if(!c){return}var b=this.find(d);if(!b){this.appendChild(c)}this._childList.before(b.ele);c.parent=this;c.depth=this.depth+1;this._children.push(c);c.setTree(this.tree)},removeChild:function(c){var b=this.find(c);if(b!=null){b.remove()}},find:function(h,b,f){if(!this._children.length){return null}if(!f){f=this.tree.idKey}var d=null;for(var e=0,g=this._children.length;e<g;e++){if(this._children[e].get(f)==h){return this._children[e]}if(b){d=this._children[e].find(h,true,f);if(d!=null){return d}}}return null},search:function(b){var j=[];for(var f=0,m=this._children.length;f<m;f++){var g=0,n=0;for(var e in b){n++;if(this._children[f]&&b[e]==this._children[f].get(e)){g++}}if(g>0&&g==n){j.push(this._children[f])}var h=this._children[f].search(b);for(var e=0,d=h.length;e<d;e++){j.push(h[e])}}return j},getChildren:function(d){var g=[];for(var f=0,j=this._children.length;f<j;f++){if(!this._children[f]){continue}g.push(this._children[f]);if(d){var h=this._children[f].getChildren(true);for(var e=0,b=h.length;e<b;e++){g.push(h)}}}return g},remove:function(){this.ele.remove()},hide:function(){this.ele.hide()},show:function(){this.ele.show()},isShow:function(){return this.ele.is(":visible")},get:function(b){return this.data[b]!=undefined?this.data[b]:null},getData:function(){return this.data},setTree:function(b){var c=(this.tree!=b);this.tree=b;this.baseCls=this.tree.baseCls;if(c){this.init()}},expand:function(){this.isExpanded=true;this.ele.addClass(this.baseCls+"-node-expand");this.nodeBody.find("."+this.baseCls+"-ec-icon").removeClass(this.baseCls+"-elbow-plus").addClass(this.baseCls+"-elbow-minus");if(this.parent){this.parent.expand()}this._childList.show();this.triggerEvent("expand")},collspan:function(){delete this.isExpanded;this.ele.removeClass(this.baseCls+"-node-expand");this.nodeBody.find("."+this.baseCls+"-ec-icon").removeClass(this.baseCls+"-elbow-minus").addClass(this.baseCls+"-elbow-plus");this._childList.hide();this.triggerEvent("collspan")},toggle:function(){if(this.isExpanded){this.collspan()}else{this.expand()}},select:function(){this.ele.addClass(this.baseCls+"-node-selected")},unselect:function(){this.ele.removeClass(this.baseCls+"-node-selected")},bind:function(b,c){if(b=="expand"||b=="collspan"){if(typeof(c)=="function"){if(undefined==this.event[b]){this.event[b]=[]}this.event[b].push(c)}}},unbind:function(b){undefined===this.event[b]||delete this.event[b]},triggerEvent:function(b){if(undefined===this.event[b]){return}for(var d=0,e=this.event[b].length;d<e;d++){if(typeof(this.event[b][d])=="function"){this.event[b][d].call(this)}}},_formatEl:function(h){var g=new RegExp("{([^}]+)}","g");var b=null,d=null,f=[];while(b=g.exec(h)){f.push(b)}for(var e=0,j=f.length;e<j;e++){d=this.get(f[e][1]);d=d==null?"":d;h=h.replace(f[e][0],d)}return h}}})(jQuery);
/* window */
