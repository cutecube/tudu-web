 ...
 * @version    $Id: Tudu.php 2758 2013-02-27 06:15:56Z cutecube $
 */
    ...
    public function createTudu($params)
    {
        ...
        if (!empty($params['flowid']) && !empty($params['nstepid'])) {
            ...
            if (null != $flow) {
                ...
                $names = array();
                foreach ($users as $user) {
                    list(, $name) = explode(' ', $user['userinfo']);
                    $names[] = $name;
                }
                $names = implode(',', $names);
...
POST;
    ...
    public function reviewTudu($params)
    {
        ...
        $to = array();
        if ($agree) {
            $currStep = $daoStep->getStep(array('tuduid' => $tuduId, 'stepid' => $tudu->stepId));
    ...
    public function cycle($params)
    {
        ...
        if ($params['type'] == 'task' && $tuduId) {
            ...
            if ($step && $step->type == Dao_Td_Tudu_Step::TYPE_CLAIM) {
                $claimUsers = $daoStep->getUsers($fromTuduId, $step->stepId);
                ...
                if ($ret) {
                    foreach ($claimUsers as $item) {
                        $daoStep->addUser(array(
                            ...
                            'status'       => (int) $item['processindex'] == 1 ? 1 : 0
                        ));
                        $to[] = $item['userinfo'];
                    }
                    // 整理认领接收人
                    $toUsers = Tudu_Tudu_Storage::formatRecipients(implode("\n", $to));
                    $addressBook = Tudu_AddressBook::getInstance();
                    foreach ($toUsers as $key => $item) {
                        ...
                        $user['role']         = Dao_Td_Tudu_Tudu::ROLE_ACCEPTER;
                        $user['tudustatus']   = Dao_Td_Tudu_Tudu::STATUS_UNSTART;
                        $user['percent']      = 0;
                        if ($isAuth || (!empty($user['isforeign']) && $isAuth)) {
                            $user['authcode'] = Oray_Function::randKeys(4);
                        }
                ...
                // 审批
                if ($step && $step->type == Dao_Td_Tudu_Step::TYPE_EXAMINE) {
                    $reviewer = $daoStep->getUsers($fromTuduId, $step->stepId);
                ...
                $stepId = $daoStep->createStep(array(
                    'orgid'      => $tudu->orgId,
                    'stepid'     => $stepId,
                    'tuduid'     => $tuduId,
                    'uniqueid'   => $fromUnId,
                ...
                foreach ($recipients as $unId => $recipient) {
                    if (isset($recipient['role']) && $recipient['role'] == 'to') {
                        $daoStep->addUser(array(
                            'tuduid'       => $tuduId,
                            'stepid'       => $stepId,
                            'uniqueid'     => $unId,
                            'userinfo'     => $recipient['accepterinfo'],
                            'processindex' => 0,
                            'status'       => 0
                        ));
                    }
