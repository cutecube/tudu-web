 ...
 * @author     Oray-Yongfa
 * @version    $Id: Flow.php 2484 2012-12-07 10:36:54Z cutecube $
 */
...
class Dao_Td_Flow_Flow extends Oray_Dao_Abstract
{
    ...
    public static function formatSteps($str)
    {
        ...
        $steps = simplexml_load_string($str);
        ...
        foreach ($steps->steps as $step) {
            foreach ($step as $val) {
                $item = array(
                    ...
                    'users' => (string) $val->users,
                    'username' => self::formatUserName((string) $val->users),
                    'prev' => isset($val->prev) ? (string) $val->prev : null,
                    'next' => isset($val->next) ? (string) $val->next : null,
                    'description' => !empty($val->description) ? trim((string) $val->description) : null
                );
                $ret[] = $item;
            }
        }
     ...
     * @param string $address
     */
    public static function formatUserName($address)
    {
        $name = array();
        $users = explode("\n", $address);
    ...
    public static function formatData($data)
    {
        ...
        foreach ($data as $key => $val) {
            ...
            if ($key == 'users' || $key == 'subject' || $key == 'description') {
