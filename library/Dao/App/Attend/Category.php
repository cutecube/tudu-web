 ...
 * @version    $Id: Category.php 2761 2013-02-28 10:21:11Z chenyongfa $
 */
...
class Dao_App_Attend_Category extends Oray_Dao_Abstract
{
    ...
    public static function formatData($data)
    {
        ...
        foreach ($data as $key => $val) {
            ...
            if ($key == 'branches' && isset($data[$key])) {
                $ret[] = '<'.$key.'>' . self::formatBraches($data[$key]) . '</'.$key.'>';
            } elseif ($key == 'users' && isset($data[$key])) {
    ...
    public static function formatSteps($str)
    {
        ...
        $flow = @simplexml_load_string($str);
        if ($flow) {
            $ret['flowid'] = (string) $flow->attributes()->id;

            $steps = array();
            foreach ($flow->steps as $step) {
                foreach ($step as $val) {
                    $item = array();
                    $item['id'] = (string) $val['id'];

                    if (isset($val->branches)) {
                        foreach ($val->branches as $branch) {
                            foreach ($branch as $v) {
                                $item['branches'][] = array(
                                    'type'  => (int) $v->type,
                                    'users' => (string) $v->users,
                                    'start' => isset($v->start) ? (string) $v->start : null,
                                    'end'   => isset($v->end) ? (string) $v->end : null
                                );
                            }
                        }
                    } else {
                        $item['type']  = (int) $val->type;
                        $item['users'] = (string) $val->users;
                    }

                    $item['next'] = isset($val->next) ? (string) $val->next : null;
