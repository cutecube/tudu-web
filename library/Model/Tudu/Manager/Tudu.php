 ...
 * @version    $Id: Tudu.php 2590 2012-12-31 10:04:53Z cutecube $
 */
...
class Model_Tudu_Manager_Tudu extends Model_Abstract
{
    ...
    public function star(array $params)
    {
        ...
        if ($success <= 0) {
            require_once 'Model/Tudu/Exception.php';
            throw new Model_Tudu_Exception('Mark star failed', self::CODE_SAVE_FAILED);
        }
    }
...
HTML;
    ...
    public function claim(array $params)
    {
        ...
        // 获取步骤
        $step = $manager->getStep($tuduId, $tudu->stepId);
        // 判断当前是否为认领操作
        ...
        $ret = $manager->claimTudu($tuduId, $orgId, $uniqueId);
        if (!$ret) {
            require_once 'Model/Tudu/Exception.php';
            throw new Model_Tudu_Exception('Reject tudu failed', self::CODE_SAVE_FAILED);
        }
        ...
        if (empty($params['postparams'])) {
            require_once 'Model/Tudu/Exception.php';
            throw new Model_Tudu_Exception('Missing or invalid value of post parameter', self::CODE_MISSING_POST_PARAMS);
        }
        $postId = $manager->createPost($params['postparams']);
        //发送回复
        $manager->sendPost($tuduId, $postId);
        //标记未读
        $manager->markAllUnRead($tuduId);
...
//     }
    ...
    protected function _writeLog($targetType, $targetId, $action, array $params, $detail = null, $privacy = false, $isSystem = false)
    {
        ...
        $daoLog->createLog(array(
            'orgid'      => $params['orgid'],
            'uniqueid'   => $params['uniqueid'],
            'operator'   => $isSystem ? '^system 图度系统' : (isset($params['userinfo']) ? $params['userinfo'] : $this->_user->userName . ' ' . $this->_user->trueName),
            'logtime'    => time(),
